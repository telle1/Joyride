...
function CurrentDrive({ currentDrive, setShowAlert, fetchDrives }) {
  ...
  return (
    <tr>
      <td>
        <p>{currentDrive.date} </p>
        <React.Fragment>
          {/* <div>RIDE ID{currentDrive.ride_id}</div> */}
          <button className='btn btn-theme mr-2' onClick={handleEditShow}>
            Edit
          </button>
          <button className='btn btn-yellow mr-2' onClick={handleManageShow}>
            Manage
          </button>
      ...
      <td>
        {currentDrive.start_loc} -> <br />
        {currentDrive.end_loc}
      </td>
      <td>{currentDrive.seats}</td>
      <td>${currentDrive.price}</td>
      <td>
        <PassengersList currentDrive={currentDrive} />
      </td>
      <td>
        <RequestsList
          currentDrive={currentDrive}
  ...
  return (
    <React.Fragment>
      <a href='#' onClick={handleInfoShow}>
        {request.name[0]} {request.name[1]} ({request.seats_requested}){' '}
      </a>
      {/* ID:{request.id} */}
      <ContactInfoModal
        showInfo={showInfo}
...
function PassengersList({ currentDrive }) {
  return (
    <React.Fragment>
      {currentDrive.passengers.map((passenger) => (
...
function Passenger({ passenger }) {
  ...
  return (
    <React.Fragment>
      <a href='#' onClick={handleInfoShow}>
        {passenger.name[0]} {passenger.name[1]} ({passenger.seats_requested})
      </a>
      <ContactInfoModal
        showInfo={showInfo}
        handleInfoClose={handleInfoClose}
